Feature: Common
  ...
  Scenario: Store a UUID in the context
    ...
    Given the HTTP endpoint "[CONF:httpbin.url]/anything/[CTXT:test.uuid]/[CTXT:test.value]"
     ...
     Then the HTTP status code must be "200"
      And the HTTP response body must have the JSON properties
          | method | GET                                                      |
          | url    | [CONF:httpbin.url]/anything/[CTXT:test.uuid]/[SHA256:test.value] |
  ...
  Scenario: Parse URL
    Given I parse the URL "https://www.elevenpaths.com:443/products-services/solutions?a=1&b=test" in context "url"
     Then the value "[CTXT:url.scheme]" must be equal to "https"
      And the value "[CTXT:url.httpbin.host]" must be equal to "www.elevenpaths.com:443"
      And the value "[CTXT:url.httpbin.hostname]" must be equal to "www.elevenpaths.com"
