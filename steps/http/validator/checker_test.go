...
import (
	...
	httpGolium "github.com/TelefonicaTC2Tech/golium/steps/http"
...
func TestReplaceMapStringResponse(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			ctx := httpGolium.InitializeContext(context.Background())
			j := JSONService{}
			...
			bodyContent, _ := httpGolium.GetParamFromJSON(ctx, tc.file, tc.code, "response")
